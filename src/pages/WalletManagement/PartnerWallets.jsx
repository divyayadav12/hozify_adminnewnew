...
import { Search, Eye, RefreshCw, AlertTriangle, Briefcase } from 'lucide-react';
...
export default function PartnerWallets() {
  ...
  const partnerWallets = mockWallets.filter(w => w.type === 'Partner');

  const filteredWallets = partnerWallets.filter(w => {
  ...
  return (
    <AdminShell
      ...
      headerTitle="Partner Wallets"
      searchPlaceholder="Search partner wallets..."
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', padding: '24px 0' }}>
        ...
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '800', color: 'var(--text)', margin: 0 }}>
            Partner Wallets Directory
          </h1>
          <p style={{ fontSize: '13px', color: 'var(--muted)', margin: '4px 0 0' }}>
            Verify split transaction payouts, monthly service earnings, and security lockups for regional and global partners.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          <div className="panel" style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ height: '36px', width: '36px', borderRadius: '50%', background: '#ecfdf5', color: '#07956f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Briefcase size={18} />
            </div>
            <div>
              <span style={{ fontSize: '10px', fontWeight: '800', color: 'var(--muted)', textTransform: 'uppercase' }}>Active Partner Wallets</span>
              <strong style={{ display: 'block', fontSize: '20px', fontWeight: '850', color: 'var(--text)', marginTop: '2px' }}>
                {partnerWallets.length} Accounts
              </strong>
            </div>
          </div>
          <div className="panel" style={{ padding: '16px' }}>
            <span style={{ fontSize: '10px', fontWeight: '800', color: 'var(--muted)', textTransform: 'uppercase' }}>Partner Lifetime Splits</span>
            <strong style={{ display: 'block', fontSize: '20px', fontWeight: '850', color: '#25108f', marginTop: '4px' }}>
              ₹{partnerWallets.reduce((sum, w) => sum + w.lifetimeEarnings, 0).toLocaleString('en-IN')}
            </strong>
          </div>
          <div className="panel" style={{ padding: '16px' }}>
            <span style={{ fontSize: '10px', fontWeight: '800', color: 'var(--muted)', textTransform: 'uppercase' }}>Avg Partner Risk Score</span>
            <strong style={{ display: 'block', fontSize: '20px', fontWeight: '850', color: '#07956f', marginTop: '4px' }}>
              {(partnerWallets.reduce((sum, w) => sum + w.riskScore, 0) / partnerWallets.length).toFixed(1)}%
            </strong>
          </div>
        </div>
        ...
        <div className="panel" style={{ padding: '16px', display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', border: '1px solid var(--line)', background: '#fff', height: '36px', padding: '0 12px', borderRadius: '6px', flex: 1, minWidth: '200px' }}>
            ...
            <input
              placeholder="Search partner owner or ID..."
              value={searchTerm}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
              <thead>
                <tr style={{ background: '#f4eff8', borderBottom: '1px solid var(--line)' }}>
                  <th style={{ padding: '12px 16px', color: 'var(--muted)', fontWeight: '800', textTransform: 'uppercase', fontSize: '10px' }}>Partner ID</th>
                  <th style={{ padding: '12px 16px', color: 'var(--muted)', fontWeight: '800', textTransform: 'uppercase', fontSize: '10px' }}>Partner Name</th>
                  <th style={{ padding: '12px 16px', color: 'var(--muted)', fontWeight: '800', textTransform: 'uppercase', fontSize: '10px' }}>Available Split Balance</th>
                  <th style={{ padding: '12px 16px', color: 'var(--muted)', fontWeight: '800', textTransform: 'uppercase', fontSize: '10px' }}>Lifetime Share</th>
                  <th style={{ padding: '12px 16px', color: 'var(--muted)', fontWeight: '800', textTransform: 'uppercase', fontSize: '10px' }}>Total Payouts</th>
