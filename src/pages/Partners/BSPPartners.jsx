...
=======
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500 border-b border-slate-100">
                <tr>
                  <th className="px-6 py-4">PROVIDER / ENTITY ID</th>
                  <th className="px-6 py-4">REGIONAL HUB / SERVICE</th>
                  <th className="px-6 py-4">TRACKED REVENUE</th>
                  <th className="px-6 py-4">STATUS</th>
                  <th className="px-6 py-4 text-center">KYC DOCUMENTS</th>
              ...
              <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                {filteredPartners.length > 0 ? (
                  filteredPartners.map((partner) => {
                    const isKycExpanded = expandedPartnerId === partner.id;
                    return (
                      <React.Fragment key={partner.id}>
                        <tr className={`transition hover:bg-slate-50/70 ${isKycExpanded ? "bg-indigo-50/20" : ""}`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap align-middle">
                            ...
                            <div className="text-xs text-indigo-600 font-semibold">{partner.serviceType || "Corporate Lease"}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap align-middle font-semibold text-slate-800">
                            {formatIndianCurrency(partner.revenue)}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap align-middle">
                          ...
                          {/* KYC Toggle Button Trigger */}
                          <td className="px-6 py-4 whitespace-nowrap align-middle text-center">
                            <button 
                              onClick={() => togglePartnerKyc(partner.id)}
                              className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl border transition shadow-sm ${
                                isKycExpanded 
                            ...
                            >
                              ...
                              {isKycExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                            </button>
                          </td>
                        ...
                        {/* Collapsible Personal KYC & Business KYC Row Container */}
                        {isKycExpanded && (
                          <tr>
                            <td colSpan="6" className="p-0 bg-slate-50/50">
                              <div className="px-8 py-6 border-l-4 border-indigo-500 bg-white grid grid-cols-1 md:grid-cols-2 gap-8 shadow-inner animate-fadeIn">
                                
                                {/* Personal KYC Card Section */}
                                <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm space-y-4">
                                  <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100">
                                    ...
                                    <div>
                                      <h5 className="text-xs font-extrabold text-slate-900 uppercase tracking-wider">Personal KYC</h5>
                                      <p className="text-[10px] text-slate-400 font-medium">Individual identity & document mapping records</p>
                                    </div>
                                  </div>
                                  <div className="space-y-2">
                                    <KycDocumentRow 
                                      ...
                                      onClick={() => openKycModal(partner.id, "personalKyc", "aadhaar", "Aadhaar Signature Verification")}
                                    />
                                    <KycDocumentRow 
                                      label="Personal PAN"
                                      docData={partner.personalKyc?.pan}
                                      onClick={() => openKycModal(partner.id, "personalKyc", "pan", "Personal PAN Card")}
                                    />
                                  </div>
                                </div>
